...
document.addEventListener('DOMContentLoaded', () => {
  ...
  otpForm.addEventListener('submit', async (e) => {
    e.preventDefault();
    const token = Array.from(otpInputsContainer.children)
    ...
    try {
      ...
      if (error) throw error;
      showNotification(
        'Verifikasi berhasil! Anda akan diarahkan ke halaman utama.'
      );
      setTimeout(() => {
        window.location.href = 'index.html';
      }, 2000);
  ...
  requestResetForm.addEventListener('submit', async (e) => {
    ...
    const submitButton = requestResetForm.querySelector(
      'button[type="submit"]'
    );
    submitButton.disabled = true;
  ...
  verifyResetOtpForm.addEventListener('submit', async (e) => {
    e.preventDefault();
    const token = Array.from(resetOtpInputsContainer.children)
      .map((input) => input.value)
      .join('');
    if (token.length !== 6) {
      showNotification('Harap masukkan 6 digit kode OTP.', 'error');
    ...
    const submitButton = verifyResetOtpForm.querySelector(
      'button[type="submit"]'
    );
    submitButton.disabled = true;
    submitButton.textContent = 'Memverifikasi...';
