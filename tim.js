...
const fetchAndRenderTeam = async () => {
    const coreTeamContainer = document.getElementById('core-team-list');
    const regionalTeamContainer = document.getElementById('regional-team-list');

    coreTeamContainer.innerHTML = '<p class="text-center">Memuat...</p>';
    regionalTeamContainer.innerHTML = '<p class="text-center">Memuat...</p>';
    ...
    try {
        ...
        const coreTeam = data.filter(member => member.category === 'Tim Inti');
        const regionalTeam = data.filter(member => member.category === 'Tim Regional');

        // Render Tim Inti
        if (coreTeam.length > 0) {
            coreTeamContainer.innerHTML = coreTeam.map(member => `
                <div class="bg-white p-4 rounded-lg shadow-lg text-center transform hover:-translate-y-2 transition-transform duration-300 w-60">
                    <div class="w-full aspect-square bg-gray-200 rounded-md mb-4 overflow-hidden flex items-center justify-center">
                         <img src="${member.photo_url || 'https://placehold.co/400x400/dcfce7/1e293b?text=Foto'}" alt="${member.name}" class="w-full h-full object-contain">
                    </div>
                    <h3 class="font-bold text-lg text-sankara-dark">${member.name}</h3>
                    <p class="text-sm text-gray-500">${member.position}</p>
                </div>
            `).join('');
        } else {
            coreTeamContainer.innerHTML = '<p class="text-center text-gray-500">Data Tim Inti belum tersedia.</p>';
        }

        // Render Tim Regional
        if (regionalTeam.length > 0) {
            regionalTeamContainer.innerHTML = regionalTeam.map(member => `
                 <div class="bg-white p-4 rounded-lg shadow-lg text-center transform hover:-translate-y-2 transition-transform duration-300 w-60">
                    <div class="w-full aspect-square bg-gray-200 rounded-md mb-4 overflow-hidden flex items-center justify-center">
                         <img src="${member.photo_url || 'https://placehold.co/400x400/dcfce7/1e293b?text=Foto'}" alt="${member.name}" class="w-full h-full object-contain">
                    </div>
                    <h3 class="font-bold text-lg text-sankara-dark">${member.name}</h3>
                    <p class="text-sm text-gray-500">${member.position}</p>
                    <p class="text-xs text-sankara-green-dark font-semibold mt-1">${member.region || ''}</p>
                </div>
            `).join('');
        } else {
            regionalTeamContainer.innerHTML = '<p class="text-center text-gray-500">Data Tim Regional belum tersedia.</p>';
        }

    } catch (error) {
        console.error("Gagal memuat data tim:", error);
        coreTeamContainer.innerHTML = '<p class="text-center text-red-500">Gagal memuat data tim.</p>';
        regionalTeamContainer.innerHTML = '';
    }
};


// --- FUNGSI BARU UNTUK MEMUAT FOOTER ---
const setupFooter = () => {
...
document.addEventListener('DOMContentLoaded', () => {
    setupAuthUI();
    setupUIInteractions();
    setupFooter(); // Memanggil fungsi footer
    fetchAndRenderTeam();
});
