...
// --- LOGIKA HALAMAN LAPORAN ---
const fetchAndRenderReports = async () => {
    const container = document.getElementById('reports-list-container');
    if (!container) return;
    container.innerHTML = '<p class="text-center">Memuat laporan...</p>';
    ...
    try {
        ...
        if (data.length === 0) {
            container.innerHTML = '<p class="text-center text-gray-500">Belum ada laporan tahunan yang dipublikasikan.</p>';
            return;
        }
        ...
        container.innerHTML = data.map(report => `
            <div class="bg-white p-4 rounded-lg shadow-md flex items-center justify-between">
                <div class="flex items-center">
                    <svg class="w-8 h-8 text-sankara-green-dark mr-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                    <div>
                        <h3 class="font-bold text-lg text-sankara-dark">${report.title || `Laporan Tahunan ${report.year}`}</h3>
                        <p class="text-sm text-gray-500">Tahun ${report.year}</p>
                    </div>
                </div>
                <a href="${report.file_url}" download target="_blank" class="bg-sankara-dark text-white text-sm font-bold py-2 px-4 rounded-lg hover:bg-black transition-colors">
                    Unduh
                </a>
            </div>
        `).join('');
        ...
        container.innerHTML = '<p class="text-center text-red-500">Gagal memuat laporan.</p>';
    }
};
